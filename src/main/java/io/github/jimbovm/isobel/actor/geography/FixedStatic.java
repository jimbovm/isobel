...
import io.github.jimbovm.isobel.bytecode.geography.DTypeCommand;
...
public final class FixedStatic extends GeographyActor {
	...
	public enum Type {
		...
		/**
		 * Spawns infinite swimming Cheep-Cheeps underwater, and infinite Bullet Bills
		 * elsewhere.
		 */
		INFINITE_BULLET_BILL_GENERATOR(DTypeCommand.ActorId.INFINITE_BULLET_BILL_GENERATOR.getId()),
		/**
		 * "Off switch" for infinite enemy generators, plus "back off" command for
		 * Lakitu.
		 */
		...
		static {
			for (Type type : Type.values()) map.put(type.id, type);
		}
	 ...
	 * 
	 * @return      A new instance of <code>FixedStatic</code> with the given
	 *              parameters.
	 */
