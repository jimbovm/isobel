...
public final class GeographyParser extends BytecodeParser<GeographyActor> {
	...
	private GeographyActor parseCommand(final int lowByte, final int highByte) {
		...
		log
			.info(String
				.format("Parsing geography command from bytes %02x %02x (%s %s)", lowByte, highByte,
		...
		switch (commandType) {
			case 0xC:
				return CTypeCommand.parse(lowByte, highByte, page);
			case 0xD:
				return DTypeCommand.parse(lowByte, highByte, page);
			case 0xE:
				return ETypeCommand.parse(lowByte, highByte, page);
			case 0xF:
				return FTypeCommand.parse(lowByte, highByte, page);
			default:
				return NormalCommand.parse(lowByte, highByte, page);
		}
	}
