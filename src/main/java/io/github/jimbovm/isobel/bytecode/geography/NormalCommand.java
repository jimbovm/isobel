...
import static io.github.jimbovm.isobel.bytecode.common.CommandUtils.*;
...
public final class NormalCommand {
	...
	public enum PrimeActorId {
		...
		/**
		 * A ? Block containing a Super Mushroom or Fire Flower, depending on the
		 * player's state.
		 */
		...
		HIDDEN_BLOCK_1UP(3),
		/**
		 * A brick block containing a Super Mushroom or Fire Flower, depending on the
		 * player's state.
		 */
		BRICK_POWERUP(4),
		...
		BRICK_1UP(8),
		/**
		 * A short sideways pipe, always enterable. Used as the exit from underwater
		 * areas in the original.
		 */
		SIDEWAYS_PIPE(9),
	...
	public enum AltActorId {
		...
		/**
		 * A tree, giant mushroom or Bullet Bill cannon depending on the area header's
		 * settings.
		 * 
		 * @see io.github.jimbovm.isobel.common.AreaHeader.Platform
		 */
		...
		static {
			for (AltActorId type : AltActorId.values()) map.put(type.id, type);
		}
		 ...
		 */
		public static AltActorId from(int id) {
			return map.get(id);
		}
	}
	...
	private static Row parseRow(final int x, final int y, final int highByte) {
		...
		switch (actorId) {
			case ROW_BRICKS:
				row.setType(Row.Type.BRICK);
				break;
			case ROW_BLOCKS:
				row.setType(Row.Type.BLOCK);
				break;
			case ROW_COINS:
				row.setType(Row.Type.COIN);
				break;
			default:
				throw new IllegalStateException(
					"Attempt to create a row with invalid type code " + actorId);
		}
	...
	private static Column parseColumn(final int x, final int y, final int highByte) {
		...
		switch (actorId) {
			case COLUMN_BRICKS:
				column.setType(Column.Type.BRICK);
				break;
			case COLUMN_BLOCKS:
				column.setType(Column.Type.BLOCK);
				break;
			default:
				throw new IllegalStateException(
					"Attempt to create a row with invalid type code " + actorId);
	...
	private static GeographyActor parseAltActor(final int x, final int y, final int highByte) {
		...
		switch (altActor) {
			case EXTENSIBLE_PLATFORM:
				parsed = parseExtensiblePlatform(x, y, highByte);
				break;
			case ROW_BLOCKS:
			case ROW_BRICKS:
			case ROW_COINS:
				parsed = parseRow(x, y, highByte);
				break;
			case COLUMN_BRICKS:
			case COLUMN_BLOCKS:
				parsed = parseColumn(x, y, highByte);
				break;
			case UPRIGHT_PIPE:
				parsed = parseUprightPipe(x, y, highByte);
				break;
			default:
				System.out.println(altActor.toString());
				parsed = null;
		}
	 ...
	 * 
	 * @return         A two-byte array of bytecode recognisable by the original
	 *                 game.
	 */
